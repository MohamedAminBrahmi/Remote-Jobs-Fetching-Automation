...
What this repo does
Calls the RemoteOK and Remotive APIs to pull all currently active remote job postings.
Saves each source's response as a raw, untouched CSV snapshot (`remoteok_jobs_raw.csv`, `remotive_jobs_raw.csv`).
Runs on a daily schedule via an Airflow DAG, so each run adds one more day of real historical data.
...
Place the contents of `airflow/` in your Airflow `dags/` folder (or mount it if using Docker).
Start Airflow (`docker compose up` if using the standard Airflow Docker setup).
In the Airflow UI, locate the `remote_jobs_fetching_notebook` DAG and toggle it on.
It will run automatically every day at 10:00 based on the `0 10 * * *` schedule — no manual triggering needed going forward.
